...
using DebugCrawlerHost = CrawlerIntegrationTesting.CrawlerHost.DebugCrawlerHost<CluedIn.Crawling.Sterling.Core.SterlingCrawlJobData>;

namespace CluedIn.Crawling.Sterling.Integration.Test
{
    public class SterlingTestFixture
    {
        public ClueStorage ClueStorage { get; }
        private readonly DebugCrawlerHost debugCrawlerHost;

        public ILogger<SterlingTestFixture> Log { get; }

        public SterlingTestFixture()
        {
            var executingFolder = new FileInfo(Assembly.GetExecutingAssembly().CodeBase.Substring(8)).DirectoryName;
            debugCrawlerHost = new DebugCrawlerHost(executingFolder, SterlingConstants.ProviderName, c => {
            ...
            Log = debugCrawlerHost.AppContext.Container.Resolve<ILogger<SterlingTestFixture>>();

            debugCrawlerHost.ProcessClue += ClueStorage.AddClue;

            debugCrawlerHost.Execute(SterlingConfiguration.Create(), SterlingConstants.ProviderId);
        }
