...
{
    public class SterlingUpdateMeshProcessor : BaseMeshProcessor
    {
        public EntityType[] EntityType { get; }
        public string EditUrl { get; }

        protected SterlingUpdateMeshProcessor(ApplicationContext appContext, string editUrl, params EntityType[] entityType)
            : base(appContext)
        {
        ...
        public override List<RawQuery> GetRawQueries(IDictionary<string, object> config, IEntity entity, Core.Mesh.Properties properties)
        {
            ...
            return new List<Core.Messages.WebApp.RawQuery>()
            {
                ...
                {
                    Query = string.Format("curl --location --request PUT '{0}' --header 'Content-Type: application/json' --header 'Authorization: Bearer {1}' --data-raw '{2}'", "https://api.us.int.sterlingcheck.app/v2/" + this.EditUrl + "/" + this.GetLookupId(entity), sterlingCrawlJobData.ApiKey, JsonUtility.Serialize(properties)),
                    Source = "cUrl"
                }
            };
        }
        ...
        public override List<QueryResponse> RunQueries(IDictionary<string, object> config, string id, Core.Mesh.Properties properties)
        {
            ...
            var request = new RestRequest(EditUrl + "/" + id, Method.PUT);
